...
public class EmaillistServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if("form".equals(action)) {
			WebUtill.forward("/WEB-INF/views/form.jsp", request, response);
			...
			WebUtill.redirect(request.getContextPath() + "/el", request, response);
		} else {
			List<EmaillistVo> list = new EmaillistDao().findAll();
			
			// forwarding = request dispatch = request extension
			request.setAttribute("list", list);
			WebUtill.forward("/WEB-INF/views/index.jsp", request, response);
		}
